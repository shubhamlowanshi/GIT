...
const WomensCollection = ({ onAddToCart }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        setWomensCollection(response.data);
      } catch (err) {
  ...
  const totalPages = Math.ceil(womensCollection.length / ITEMS_PER_PAGE);
  const paginatedItems = womensCollection.slice(
  ...
  return (
    <>
      ...
      <div style={{ backgroundColor: '#D07D9E', color: 'white', fontSize: '40px', padding: '15px' }}>
        Women's Collection
      </div>
